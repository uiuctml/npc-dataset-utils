...
def main():
    if (not os.path.isdir(header.label_dataset_dir_annotations)):
        logger.log_error("Invalid dataset annotations directory.")
        return

    dataset_dir_annotations_list = os.listdir(header.label_dataset_dir_annotations)
    ...
    for file_name_annotations in dataset_dir_annotations_list:
        ...
        file_annotations_counter += 1
        file_path_annotations = os.path.join(header.label_dataset_dir_annotations, file_name_annotations)
