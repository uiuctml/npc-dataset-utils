...
With the above parameters, `split.py` generates, compresses, and saves the new split configuration as `npc/npc-dataset-utils/configs/npc-dataset-utils/gtsrb_split.json.gz`. Additional aspects of split.py, e.g., split percentages, may also be customized via parameters in `header.py`.
